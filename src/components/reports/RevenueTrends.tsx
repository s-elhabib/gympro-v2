      ...
      <CardContent>
        {!hasData ? (
          <div className="h-[350px] w-full flex items-center justify-center flex-col">
            <p className="text-gray-500 mb-2">Aucune donnée disponible pour cette période</p>
            <p className="text-sm text-gray-400">Les données de revenus s'afficheront ici une fois disponibles</p>
          </div>
        ) : (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
          <div className="bg-blue-50 p-4 rounded-lg">
            ...
            <p className="text-2xl font-bold mt-1">
              {formatCurrency(hasData ? data.reduce((sum, item) => sum + item.revenue, 0) : 0)}
            </p>
          </div>
          ...
          <div className="bg-blue-50 p-4 rounded-lg">
            ...
            <p className="text-2xl font-bold mt-1">
              {formatCurrency(hasData ? data.reduce((sum, item) => sum + item.revenue, 0) / data.length : 0)}
            </p>
          </div>
