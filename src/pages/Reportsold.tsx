...
const Reports = () => {
  ...
  const generateReport = async () => {
    ...
    try {
      ...
      const reportName = `${activeTab}_${reportType}_${format(
        new Date(dateRange.start),
        "yyyy-MM-dd"
      )}_to_${format(new Date(dateRange.end), "yyyy-MM-dd")}`;
  ...
  const handleCustomReportGeneration = async () => {
    try {
      ...
      link.setAttribute(
        "download",
        `${reportName}_${format(new Date(), "yyyy-MM-dd")}.csv`
      );
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
  ...
  const fetchMemberStats = async () => {
    try {
      ...
      setMemberStats({
        activityRate:
          ((activeMembers?.length || 0) / (totalMembers?.length || 1)) * 100,
        retention: {
          monthly:
            ((monthlyRetention?.length || 0) / (totalMembers?.length || 1)) *
            100,
          quarterly:
            ((quarterlyRetention?.length || 0) / (totalMembers?.length || 1)) *
            100,
          yearly:
            ((yearlyRetention?.length || 0) / (totalMembers?.length || 1)) *
            100,
        },
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Consultez et exportez les rapports de votre salle de sport
          </p>
        </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Distribution des revenus selon les differents forfaits
                    d'adhesion
                  </CardDescription>
                </CardHeader>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-500">
                          Les revenus ont augmente de maniere constante de 5-10%
                          mois apres mois au cours des 6 derniers mois.
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-500">
                          Les renouvellements d'adhesion annuelle ont un taux de
                          retention de 78%, superieur a la moyenne du secteur.
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-500">
                          Les pics de revenus se produisent en janvier et
                          septembre, correspondant aux resolutions du nouvel an
                          et a la rentree scolaire.
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-500">
                          {memberStats.activityRate.toFixed(1)}% des membres
                          visitent au moins deux fois par semaine
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-500">
                          Le taux de retention mensuel est de{" "}
                          {memberStats.retention.monthly.toFixed(1)}%,
                          trimestriel de{" "}
                          {memberStats.retention.quarterly.toFixed(1)}%, et
                          annuel de {memberStats.retention.yearly.toFixed(1)}%
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-500">
                          Le groupe d'age le plus important est 25-34 ans (
                          {memberStats.demographics["25-34"]}%), suivi de 35-44
                          ans ({memberStats.demographics["35-44"]}%) et 18-24
                          ans ({memberStats.demographics["18-24"]}%)
                        </p>
                      </div>
                    </div>
                  </div>
          ...
          <div className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Créez des rapports personnalisés en sélectionnant des
                  métriques et des plages de dates
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
            ...
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500">
                  Genere le {format(new Date(), "MMMM d, yyyy")}
                </p>
              </div>
            </div>
            <Button
            ...
            <div className="flex items-center">
              ...
              <div>
                <h3 className="font-medium">
                  Resume de Frequentation des Membres
                </h3>
                <p className="text-xs text-gray-500">
                  Genere le {format(subMonths(new Date(), 1), "MMMM d, yyyy")}
                </p>
              </div>
            </div>
            <Button
              variant="ghost"
              size="sm"
              onClick={() => handleExport("member-attendance")}
            >
              <Download className="h-4 w-4" />
            </Button>
          </div>
          ...
          <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500">
                  Genere le {format(subMonths(new Date(), 3), "MMMM d, yyyy")}
                </p>
              </div>
            </div>
            <Button
              variant="ghost"
              size="sm"
              onClick={() => handleExport("q1-financial")}
            >
              <Download className="h-4 w-4" />
            </Button>
          </div>
